...
<script type="text/javascript">
	$(function() {
		$('#dg').datagrid({
			url : 'queryDepartmentByName',

			loadMsg : 'Loading....', //表示加载数据过程中显示的信息

			queryParams : {
				'departmentCustom.name' : ''
			...
			idField : 'did', //指定id为标识字段，在删除，更新的时候有用，如果配置此字段，在翻页时，换页不会影响选中的项 
			...
			toolbar : [ {
				...
				handler : function() {
					parent.$("#win").window({
						...
						width:650,
						height:600,
						content:'<iframe src="department/sendDepartmentSaveJsp" frameborder="0" width="100%" height="100%" />'
					});
				}
			}, '-', {
				iconCls : 'icon-edit',
					...
					//判断是否有选中行记录，使用getSelections获取选中的所有行 
							...
							if(rows.length == 0){
								...
								parent.$("#win").window({
									...
									width:650,
									height:600,
									content:'<iframe src="department/sendDepartmentUpdate" frameborder="0" width="100%" height="100%" />'
								});
							}
				}
			}, '-', {
					...
					if (rows.length == 0) {
						...
						$.messager.confirm("删除的确认对话框", "您确定要删除此项吗？", function(r) {
							if (r) {
								...
								$.post("departmentDeleteByIds",{ dids : ids},
									function (data) {
