	...
	<script type="text/javascript">
		$(function(){
			$('#dg').datagrid({   
			    ...
			    url:'queryStudentByName',
			    loadMsg:'Loading......',
			    queryParams:{'studentCustom.name':''},//这里不需要传具体的name，因为我们要显示所有的
			    //width:300,
				...
				pageList:[5,10,15,20],
				idField:'sid',//指定id为标识字段，在删除，更新的时候有用，如果配置此字段，在翻页时，换页不会影响选中的项
				...
				toolbar: [{
					...
					handler: function(){
						parent.$("#win").window({
							...
							width:650,
							height:600,
							content:'<iframe src="student/sendStudentSave" frameborder="0" width="100%" height="100%"/>'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							parent.$("#win").window({
								...
								width:650,
								height:600,
								content:'<iframe src="student/sendStudentUpdate" frameborder="0" width="100%" height="100%"/>'
							});
						...
						if(rows.length == 0) {
							...
							$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r){
								if (r){
								    ...
								    //2. 发送ajax请求
								    $.post("studentDeleteByIds",{sids:ids},function(data){
			    ...
			    frozenColumns:[[
					...
					{field:'sid',title:'学生编号',width:100}   
			    ]],
			    columns:[[    		           
			        {field:'name',title:'学生姓名',width:100},    
			        {field:'sex',title:'学生性别',width:100},
			        {field:'date',title:'出生日期',width:150},
			        {field:'department.name',title:'系别',width:100,
			        formatter: function(value,row,index){
			        		if(row.department != null && row.department.name != null) {
			        			return row.department.name; //如果商品类别不为空，返回商品类别
			        		} else {
			        			return "不知道什么系";
			        		}
						}	
